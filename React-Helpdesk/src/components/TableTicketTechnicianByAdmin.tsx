...
const TableTicketTechnicianByAdmin : React.FC<ModalType> = ({ AssigneeID }) => {
  ...
  const FetchAllTicket = async (authtoken: string) => {
    const response = await axios.get(`${import.meta.env.VITE_API}/ticket/read_ticket_all_technician_by_admin/${AssigneeID}`, {
      headers: {
        authtoken,
      },
    });
...
export default TableTicketTechnicianByAdmin;